...
run project binary release="0": (build project release)
    #!/usr/bin/env sh
    set -uo pipefail
    cp {{ if release != "0" { invocation_directory() / project / "release" / project } else { invocation_directory() / project / "debug" / project } }} {{ invocation_directory() / binary }}
    ./{{ binary }}

# cleans the build artifacts stored in the pwd
[arg("binary", pattern='[[:alpha:]-]*')]
[arg("project", pattern='[[:alpha:]-]*')]
[no-cd]
clean project="" binary="":
    {{ if project != "" { "rm -rf " + invocation_directory() / project } else { "" } }}
    {{ if binary != "" { "rm -rf " + invocation_directory() / binary } else { "" } }}
