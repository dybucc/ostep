#![feature(
  exit_status_error, string_from_utf8_lossy_owned, bool_to_result,
  ...
  Ok(match workspace_packages.len() {
    | 2.. => {
      let pwd = env::current_dir()
        .context("failed to fetch pwd during initialization")?;
        ...
        })
      {
        let pkg_path = pkg
          ...
          .with_context(|| {
            format!("failed while processing package: {}", pkg.name)
          })?;
        ...
        bail!(
          "cargo workspace package directory doesn't match pwd and no `-p` \
           package option was provided"
        );
      }
    },
    | 1 => {
      ...
      let target_pkg_path = target_pkg
        ...
        .ok_or(anyhow!("pkg manifest path doesn't have a parent dir"))
        .with_context(|| {
          format!("failed while processing package: {}", target_pkg.name)
        })?;
      env::set_current_dir(target_pkg_path)
        .context("failed to set pwd during initialization")
        .with_context(|| {
          format!(
            "failed to set pwd to cargo package directory: {target_pkg_path}"
          )
        })?;
  ...
  Ok(
    tests
      ...
      .try_fold(
        ...
        |(mut tests, mut current_test),
         (test_path, test_num, test_extension)| {
...
fn copy_exe<T: Display>(target_pkg: &T) -> Result<String> {
  ...
  Ok(Cow::into_owned(
    exe
      .file_name()
      .expect(
        "owing to `cargo`'s stable formatting guarantees, if the program \
         hasn't already thrown an error because the `executable` key in the \
         generated build json is missing, then surely it has produced a file \
         path with the last component being the name of the executable",
      )
      .to_string_lossy(),
  ))
}
  ...
  tests.into_iter().try_for_each(
    |Test { num, out, err, rc, run, desc, .. }| {
        ...
        .try_fold(
          ...
          |mut accum, (idx, param)| {
            accum[idx] = param.ok_or_else(|| match idx {
              ...
              | _ => unimplemented!(
                "if you hit this case, you are most likely considering more \
                 test parameters and should probably reevaluate the whole \
                 match arm and its surroundings"
              ),
            })?;
      ...
      (bin.trim_matches(['.', '/']) == exe)
        ...
        .with_context(|| {
          format!(
            "crate binary: {exe}, binary name in `.run` test file param: {bin}"
          )
        })?;
      ...
      let (real_status, real_out, real_err) = (
        output
          ...
          .ok_or_else(|| {
            anyhow!(
              "failed to parse wait status of tested binary program as exit \
               code"
            )
          })
          .with_context(|| {
            format!(
              "failed while parsing test entry {num} for pkg: {target_pkg}"
            )
          })?,
